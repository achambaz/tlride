...
alter(empirical_law, QW = QW_hat, Qbar = wrapper(Qbar_hat))
  ...
  learned_features_fixed_sample_size <-
    ...
    mutate(Qbar_hat_d = map(obs, ~ estimate_Qbar(., algorithm = working_model_Q_one)),
           Qbar_hat_e = map(obs, ~ estimate_Qbar(., algorithm = kknn_algo,
                                          trControl = kknn_control, ## Antoine: small change
                                          tuneGrid = kknn_grid))) %>%
    mutate(blip_QW_d = map2(Qbar_hat_d, obs,
                            ~ compute_Qbar_hatAW(.y$A, .y$W, .x, blip = TRUE)), ## Antoine: small change
...
if (redo_varying) {
  learned_features_varying_sample_size <- obs %>% as.tibble %>% 
    ...
    mutate(block = guided.tour.tmle::label(1:nrow(.), sample_size)) %>% ## Antoine: small change
    nest(-block, .key = "obs")
} 
