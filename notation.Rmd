...
- $\|f\|_{\infty} \equiv \sup_{o \in  \bbO} |f(o)|$, the essential supremum of
$f$, a function from $\bbO$ to $\bbR$. 
...
- $\calQ  \triangleq \{\Qbar  :  P  \in  \calM\}$,  the space  of  regression
...
- $L_{a}$, the contex-specific logistic  (or negative binomial) loss function,
  given by $-L_{a}(f)(A,W)  \triangleq A \log f(W) + (1-A)  \log (1-f(W))$ for
  any function $f:[0,1]\to[0,1]$.

- $L_{y}$, the reward-specific logistic  (or negative binomial) loss function,
  given by $-L_{y}(f)(O) \triangleq Y \log f(A,W) + (1-Y) \log (1-f(A,W))$ for 
