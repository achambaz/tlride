...
\tcg{Discuss G-computation,  i.e., plug-in estimation. Discuss  IPTW too? Show
that  it  works  for  parametric   models,  but  fails  for  machine  learning
algorithms.}

## \textdbend Analysis of G-computation estimator.

\tcg{Go  through the  expansion algebra  to generate  one-step correction/TMLE
term and  empirical process term.  Highlight that  the correction term  is the
source of  the bad behavior.  If we have all  the fitted nuisance  objects, we
could  use   the}  `compute_eif`  \tcg{function  on   the  estimated  nuisance
parameters to  illustrate that root-n  times EIF at estimated  nuisances blows
up, which in turn  causes the estimator to blow up. In this  way, we provide a
theoretical explanation and numerical depiction of why G-comp fails.}

## One-step corrections.

\tcg{Remind  the reader  about asymptotic  linearity;  that we  need to  write
estimator  minus truth  as equal  to empirical  mean of  EIF at  true nuisance
parameters  + negligible  term.  We saw  that  plug-in did  not  yield such  a
representation, but look, the one-step  estimator does. Show that the one-step
estimator corrects poor  behavior of G-computation estimator.  Discuss that we
also  now  have asymptotic  linearity  and  inference  via the  central  limit
theorem.  Discuss   standard  error   estimation.  Show   confidence  interval
coverage?}
