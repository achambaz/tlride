...
draw_from_experiment <- function(n, full = FALSE) {
  ...
  attr(obs, "qY") <- function(AW, Y, Qbar){
    ...
    shape <- ifelse(A == 0, 2, 3)
    dbeta(Y, shape1 = shape, shape2 = shape * (1 - Qbar.AW) / Qbar.AW)
  }
  ##
  return(obs)
}
...
experiment. In particular, we have included several features of $P_0$ that play 
an important role in our developments. The attribute `QW` describes the density 
of $W$, that of a Uniform(1/10, 9/10) distribution. The attribute `Gbar` describes the conditional probability of action $A = 1$ given $W$, and for $a = 0,1$ and each $w$ in the support of $W$, we denote by $\Gbar_0(a,w) = pr_{P_0}(A = a \mid W = w)$. The attribute 
`qY` describes the conditional density of $Y$ given $A$ and $W$. For each $w, a, y$, we denote by $q_{0,Y}(y, a, w)$ the conditional density of $Y$ given $A = a, W = w$ evaluated at $y$. Similarly, the attribute `Qbar` describes the conditional mean of $Y$ given $A$ and $W$, and we denote by $\Qbar_0(a,w)$ the conditional mean of $Y$ given $A = a, W = w$. 

*[David note: I revised this paragraph to include a description of the conditional density
of $Y$, which is needed to describe the quantile exercises. Also, should we consider adopting the notational convention of lower case $q$ for density (e.g., $q_W$) and upper case $q$ for cumulative distribution? I need both quantities for the quantile exercises.]*
...
\end{align} Acting as oracles, we  can compute explicitly the numerical value
of $\psi_{0}$.
*[David note: Is the first equality helpful? The preceding sentence might cause a reader to expect to see $\Qbar_0$ and $Q_{0,W}$ in the equation. So maybe we flip the order? Or remove the first equality altogether?]*
...
reinforce what it means to run the "full" experiment by setting argument `full`
...
\end{equation} 
Thus, $\psi_{0}$ describes the average difference in of the two
...
*[David note: Is it worth writing this as $E_{\bbP_{0}}(Y_1) - E_{\bbP_{0}}(Y_0)$ as well (or instead)? Maybe I am thinking too much, but for the quantile example, we compare a quantile of 
$Y_1$ to a quantile of $Y_0$ rather than describe a quantile of the difference $Y_1 - Y_0$; the latter, of course, involves cross-world distributions (i.e., the joint dist. of $(Y_1,Y_0)$)]*

To conclude this subsection, we take advantage of our status as oracles to sample
observations from the full experiment. We call `draw_from_experiment` with its argument `full` set to `TRUE` in order to numerically approximate $\psi_{0}$.  By the law of large
...
As discussed above, the quantity $\psi_0$ describes the difference in average reward if we enforce action $a = 1$ rather than $a = 0$. An alternative way to quantify the reward under  different actions is the difference in the $c$-th quantile of reward if we enforce action $a = 1$ as opposed to $a = 0$. We define $Q_{0,Y}(y, a, w) = \int_{0}^y q_{0,Y}(u, a, w) du$ as the conditional cumulative distribution of the reward $Y$ given $A$ and $W$ that is implied by $P_0$ evaluated at $y \in (0,1)$. We define the covariate-adjusted quantile reward for action $a \in \{0,1\}$ as \begin{equation} \label{def_quantile}
  \phi_{0,a}(c) := \mbox{inf}\biggl\{y : \int Q_{0,Y}(y, a, w) dQ_{0,W}(w) \ge c \biggr\} \ .
\end{equation}
The difference in $c$-th quantile rewards is $\phi_0(c) = \phi_{0,1}(c) - \phi_{0,0}(c)$. 

1. Evaluate the numerical value of $\phi_{0,a}(c)$ for $(a,c) \in \{0,1\} \times \{0.25,0.5,0.75\}$ using the appropriate attributes of `five_obs`. Based on these results, report $\phi_0(c)$ for $c \in \{0.25, 0.5, 0.75\}$. 
2. Approximate the value of $\phi_{0,a}(c)$ for $(a,c) \in \{0,1\} \times \{0.25,0.5,0.75\}$ by drawing a large sample from the "full" data experiment and using empirical quantile estimates. Confirm that your results closely match those obtained in problem 1. 
...
We continue with the quantile example from section \ref{subsec:exo:dave:one}. As above, we define $q_{Y}(y,a,w)$ to be the $(A,W)$-conditional density of $Y$ evaluated at $y$ that is implied by a generic $P \in \mathcal{M}$. Similarly, we use $Q_{Y}$ to denote the corresponding cumulative distribution function. The covariate-adjusted $c$-th quantile reward for action $a \in \{0,1\}$ may be viewed as a mapping $\Phi_{a,c}$ from $\calM$ to $[0,1]$ characterized by \begin{equation*} 
  \Phi_{a,c}(P) = \inf\biggl\{ y : \int Q_{Y}(y,a,w) dQ_W(w) \ge c \biggr\} \ . 
\end{equation*}
The difference in $c$-th quantile rewards may similarly be viewed as a mapping $\Phi_c$ from $\calM$ to $[0,1]$, characterized by $\Phi_c(P) = \Phi_{1,c}(P) - \Phi_{0,c}(P)$. 

1. Evaluate the numerical value of $\Phi_{a,c}(\Pi_0)$ for $(a,c) \in \{0,1\} \times \{0.25,0.5,0.75\}$ using the appropriate attributes of `five_obs_from_another_experiment`. Based on these results, report $\Phi_c(\Pi_0)$ for $c \in \{0.25, 0.5, 0.75\}$. 
