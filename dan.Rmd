...
every $h \in H = \interval[open]{-M^{-1}}{M^{-1}}$, we can define a law $P_{h}
\in \calM$ by setting $P_{h} \ll  P$\footnote{That is, $P_{h}$ is dominated by
$P$: if an  event $A$ satisfies $P(A)  = 0$, then necessarily $P_{h}  (A) = 0$
too.}  and

\begin{equation}\label{eq:fluct}\frac{dP_{h}}{dP}(O)      =     1      +     h
s(O),\end{equation} 
...
The  following chunk  of code  enables the  computation of  the values  of the
efficient influence curve $D^{*}(P)$ at observations drawn from $P$ (note that
it  is necessary  to  provide  the value  of  $\Psi(P)$,  or an  approximation
thereof, through argument `psi`).
...
We can use `eic` to approximate the Cramér-Rao bound at $P_{0}$:
...
intervals for  $\Psi(\Pi_{0})$ are  approximately `r round(ratio,  2)` smaller
than their counterparts for $\Psi(P_{0})$.
...
\begin{align*}\sigma_{0}(O)  &=   -  10   \sqrt{W}  A   \times  \beta_{0}(A,W)
...
direction  $s$  with  `s_draw_from_another_experiment`,  then  we  approximate
...
Equal to `r  slope_approx`, the first approximation `slope_approx`  is not too
off.
