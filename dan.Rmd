...
2.  Discuss what happens when the dimension of the (still well-specified)
...
3. Discuss  what happens when the  working model is mis-specified.   You could
use the following chunk of code:
```{r exercises-two, eval = FALSE}
...
4.   \textdbend  Drawing inspiration  from \eqref{eq:v:n:b}, one  may consider
...
estimate_Q <- function(dat, working_model_Q) {
...
predict_QAW <- function(Y, A, W, algorithm, blip = FALSE) {
  ## carries out the estimation based on 'algorithm'
...
working_model_Q_two <- list(
  ...
  formula = as.formula(
    paste("Y ~ A * (",
          paste("I(W^", seq(1/2, 5, by = 1/2), sep = "", collapse = ") + "),
          "))")
  ))
...
NP_estimate_Q <- function(dat, algorithm) {
  fit <- algorithm(dat)
  Qhat <- function(newdata) {
