...
equal to $D^*(P)$. This fact suggests concrete strategies for generating efficient 
estimators of smooth parameters.

Drawing another parallel to Euclidean geometry, recall that if $f$  
is a  differentiable mapping from $\bbR^p$ to $\bbR$, we can use a Taylor series 
...
differentiable uniformly over directions, then for any given $P \in \calM$, we can write \begin{align} \label{taylor_expansion}
  \Psi(P_0) = \Psi(P) + (P_0 - P) D^*(P) - R_2(P, P_0),
\end{align}
where $R_2(P,P_0)$ is a *remainder term* satisfying that \begin{align*}
  \frac{R_2(P, P_0)}{d(P, P_0)} \rightarrow 0 \ \mbox{as} \ d(P, P_0) \rightarrow 0 ,
\end{align*}
...
\Psi(P_0) = \Psi(P) + \Exp_{P_0}(D^*(P)(O)) - \Exp_P(D^*(P)(O)) - R_2(P, P_0).
\end{align*}
