...
R> bookdown::render_book("dan.Rmd", output_format = "bookdown::pdf_document2")
...
\tcg{Our ambition  is to present a  gentle introduction to the  inference of a
causal quantity whose  statistical analysis is typical and thus  paves the way
to more involved analyses. }
...
case,  $\calQ$  consists  of  all   functions  mapping  from  $\{0,1\}  \times
\interval{0}{1}$ to $\bbR$. We can be  our estimate $\Qbar_n$ as the algorithm
$\hat{Q}$ applied  on $P_{n}$, the  empirical measure, \textit{i.e.},  the law
consisting  in  drawing   one  among  $O_{1}$,  \ldots,   $O_{n}$  with  equal
probabilities $n^{-1}$. Thus, we can write $\Qbar_n = \hat{Q}(P_n)$.
