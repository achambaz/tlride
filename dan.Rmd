...
and `ggdag`. Assuming that there are installed  too, we can run the next chunk
of code:
...
Let us run (silently) the example built into the `guided.tour.tmle` package:
...
\Pr_{P_0}(A = a | W)$.  Obvisously, $\ell\Gbar_{0}(A,W) \equiv A\Gbar_{0}(W) +
...
we  have available,  $O_{1}$,  \ldots,  $O_{n}$, be  a  sample of  independent
observations drawn  from $P_{0}$,  and the  goal is to  estimate the  value of
