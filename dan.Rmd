...
```{r redo}
redo <- c(TRUE, FALSE)[2]
## if 'redo' then recompute 'learned_features_fixed_sample_size' and
## 'learned_features_varying_sample_size', otherwise upload them if they
...
natural to inquire  of properties this functional  \tcg{enjoys}.  For example,
we may  be interested in  asking how the value  of $\Psi(P)$ changes  for laws
that are, in a loose sense, near  to $P$ in $\mathcal{M}$. If small changes in
the  law  result in  large  changes  in the  parameter  value,  then we  might
hypothesize  that it  will  be  difficult to  produce  ``good estimators''  of
$\psi_0$. Fortunately, this turns out not to be the case for $\Psi$, and so we
say  that $\Psi$  is a  smooth parameter  mapping. This  colloquial expression
refers to  the fact  that, for  each $P \in  \calM$, if  $P_{h} \to_{h}  P$ in
$\calM$ from a direction $s$ when the  real parameter $h \to 0$ (the notion of
convergence     from    a     direction    is     formalized    in     Section
\ref{subsec:being:smooth:two}),  then not  only $\Psi(P_{h})  \to_{h} \Psi(P)$
(continuity), but also  $h^{-1} [\Psi(P_{h}) - \Psi(P)] \to_{h}  c$, where the
real number $c$ depends on $P$ and $s$ (differentiability).
...
where $s : \calO\to \bbR$ is a $P$-measurable function of $O$ such that $s(O)$
...
that  the  fluctuation  is  \textit{in  the direction}  of  $s$,  because  $s$
characterizes the local behavior of the fluctuation around $P$.
...
\calM$, there  exists a  $P$-measurable function $D^{*}(P)  : \calO  \to \bbR$
such that $\Exp_{P}(D^{*}(P)(O)) = 0$ and,  for every fluctuation $\{P_{h} : h
\in  H\}$ with  score $s$  at  $h =  0$,  the real-valued  mapping $h  \mapsto
\Psi(P_{h})$  is  differentiable   at  $h=0$,  with  a   derivative  equal  to
\begin{equation}  \label{eq:derivative}\Exp_{P}\left(D^{*}(P)(O)  s(O)\right).
\end{equation}
...
```{r one-step-one}
set_Qbar_Gbar<- function(obs, Qbar, Gbar) {
  attr(obs, "Qbar") <- Qbar
  attr(obs, "Gbar") <- Gbar
  return(obs)
}
...
```{r one-step-two}
one_realization <- learned_features_fixed_sample_size %>%
  head(1) %>% rename(data = obs)
attach(one_realization)

detach(one_realization)
```
