...
## A causal story {#subsec:causal-story}
...
## The `guided.tour.tmle` package {#subsec:package}
...
Note that additional packages are required, among which `tidyverse`, `caret`
and `ggdag`. Assuming that these are installed  too, we can run the next chunk
of code:
...
## To remove at a later stage {#subsec:remove}
...
# Reproducible experiment as a law {#subsec:as:a:law}

\tcg{Continue here!}

We are interested in a reproducible experiment. Every time this experiment is run,
it generates an observation that we call $O$. We view
$O$  as a  random variable  drawn from  *the  law of the
experiment* $P_{0}$.  We view the law as an  element of  *the
model* $\calM$. The model  is a collection of  laws. In particular, the model
contains all laws that we think may plausibly describe the law of the 
experiment. Thus, the choice of model is based on our scientific knowledge
of the  experiment. The  more we know about
the experiment,  the smaller is  $\calM$. In  all our examples, we use large models
that reflect a lack of knowledge about many aspects of the experiment. 
...
[p/(1-p)]$.  Let us take a look at `experiment`:
...
# The parameter of interest, first pass. {#subsec:parameter:first}
...
## The parameter of interest, second pass. {#subsec:parameter:second}
...
## The parameter of interest, third pass. {#subsec:parameter:third}
...
## Smooth parameters, first pass. {#subsec:being:smooth:one}
...
\@ref(subsec:being:smooth:two), and here provide an informal description
of smoothness.
...
## \textdbend Being smooth, second pass. {#subsec:being:smooth:two}
...
## Revisiting Section \@ref(subsec:being:smooth:one).
...
## \textdbend Influence functions and the efficient influence function. {#subsec:parameter:third}
...
## Linear approximations of parameters {#subsec:linearapprox}
...
## \textdbend Double-robustness {#subsec:double:robustness}
...
we  have available,  $O_{1}$,  \ldots,  $O_{n}$,  a  sample of 
observations independent drawn  from $P_{0}$,  and the  goal is to  estimate the  value of
...
1. Evaluate the plug-in estimate, if, rather than the empirical distribution, we used a maximum likelihood estimator of $Q_{0,W}$ under the assumption that $W \sim \mbox{Normal}(\mu, \sigma^2)$ where $\mu$ and $\sigma^2$ are unknown. 

2. \tcg{If we asked them to implement a regression estimator in the previous subsection, let's now have them use it to repeat our simulation and compute the G-comp estimator based on this.}
...
## \textdbend Targeted minimum loss-based estimation.
...
It thus remains to devise a strategy for ensuring that $P_n D_2^*(\hat{P}_n^*) = 0$. To achieve this, we consider an approach for revising our initial estimate of $\Qbar_0$. That is, we propose a method for selecting an estimate of $\Qbar_0$ that is "near to" $\Qbar_n$, but is such that for any law $\hat{P}_n^*$ that is compatible with this revised estimate,
$P_n D*(\hat{P}_n^*) = 0$. Because $\Qbar_n$ is our "best initial guess" for the true regression $\Qbar_0$, the new regression that we pick should be *at least as good* of an estimate of $\Qbar_0$ as $\Qbar_n$. Thus, we must first propose a way to move between regressions in our model and then we must 
propose a means of moving to a new regression that fits the data at least as 
well as $\Qbar_n$. 
...
fit of different candidate regression functions to the true regression function $\Qbar_0$. Because, $\bbD_0$ depends on $P_0$, it is often unknown to us in practice. Thus, we may instead compute an empirical estimate of $\bbD_0$, \[
...
What about repeating this process of defining a submodel and searching over this submodel 
to obtain a better fitting regression? Perhaps by iterating this procedure, we could 
...
over $\delta \in \bbR$. This is straightforward since there is a one-to-one correspondence 
between the parameter $\epsilon$ indexing the submodel $\calQ(\Gbar_n)$ and 
$\calQ_1(\Gbar_n)$. In particular, for a given $\epsilon$, there is some $\delta \in \bbR$ 
such that $\delta = \epsilon - \epsilon_n$. That is to say that the choice of $\delta$ 
that minimizes $\bbD_n(\Qbar_{nm,\delta})$ is $\delta = 0$. Thus, $\Qbar_{n,\delta = 0} \equiv \Qbar_{n,\epsilon_n}$ is the best fit that we can obtain for $\Qbar_0$ by moving through the
model space via submodels of the form $\calQ(\Gbar_n)$.

Now, we hope that we have achieved our goal of finding a new regression fit that is *at least as good* as our initial estimate. However, what about the second goal of generating an estimate
of $\Qbar_0$ such that for any $\hat{P}_n^*$ that is compatible with such an estimate we have
that $P_n D*(\hat{P}_n^*) = 0$? Consider that because the minimum value of $\bbD_n(\Qbar_{n,\delta})$ is achieved at $\delta = 0$, it must be true that the derivative of $\bbD_n(\Qbar_{n,\delta})$ with respect to $\delta$ is zero when evaluated at $\delta = 0$. That is, \begin{equation} \label{deriv_equals_zero}
...
$P_n D_{2}^{*}(\hat{P}^*_n)$ for a $\hat{P}_n$ that is compatible with $\Qbar_{n,\epsilon_n}$ and $\Gbar_n$. But then equation (\ref{deriv_equals_zero}) also implies that $P_n D_{2}^{*}(\hat{P}^*_n) = 0$! What happened here? Recalling Section \ref{subsec:being:smooth:one} there were many possible submodels that we may choose to move through our model space. 
...
## Inference assuming $\Gbar_{0}$ known, or not, first pass. {#subsec:known:gbar:one}
...
## Inference assuming $\Gbar_{0}$ known, or not, second pass. {#subsec:known:gbar:two}
...
function `estimate_G`  fits a user-specified  working model by  minimizing the
...
## Inference based on the estimation of $\Qbar_{0}$. {#subsec:inf:Q0}
...
## One-step estimation. {#subsec:one:step}
...
## Targeted inference. {#subsec:tmle}
