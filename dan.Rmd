...
\tcg{I am suggesting that we now start a new section on building estimators. Thus,
everything in section 2 is about parameters, efficiency theory etc..., i.e., building
up the theoretical tools we need to construct estimators. Now Section 3 will all be
about strategies for estimation. Tentative outline (definitely open for discussion):
Sec. 3.1: Discuss how we can do the above expansion at a $\hat{P}_n$ and go through
the exercise to isolate $P_n D^*(\hat{P}_n)$. Discuss various strategies for dealing
with it (i.e., one-step, TMLE, maybe an aside about estimating equations). 
Sec 3.2: Revisit double-robustness with the below comments?
Sec 3.3: Discuss IPTW estimators with known/unknown $\Gbar$?
Sec 3.4: Discuss inference for all the estimators? 
Sec 3.5: Briefly mention other extensions? Double-robust inference?}
