...
It   is  straightforward   to  [show   that](#identification)  \begin{equation}
(\#eq:psi-zero-bis) \psi_{0}  = \Exp_{\bbP_{0}}  \left(Y_{1} -  Y_{0}\right) =
\Exp_{\bbP_{0}}(Y_1) - \Exp_{\bbP_{0}}(Y_0).   \end{equation} Thus, $\psi_{0}$
describes the average difference of  the two counterfactual rewards.  In other
words, $\psi_{0}$ quantifies the difference in average of the reward one would
get in a world where one would always enforce action $a=1$ with the reward one
would get in a world where one  would always enforce action $a=0$.  This said,
it is worth emphasizing that $\psi_{0}$ is a well-defined parameter beyond its
causal  interpretation,  and  that  it describes  a  standardized  association
between the action $A$ and reward $Y$.
...
action  $a  \in  \{0,1\}$  and   $c  \in  ]0,1[$,  introduce  \begin{equation}
\gamma_{0,a,c}  \equiv  \inf  \left\{y  \in  ]0,1[ :  \int  Q_{0,Y}(y,  a,  w)
dQ_{0,W}(w) \ge c \right\}. (\#eq:def-quantile)\end{equation}
...
$P_{0} = P_{\theta_{0}}$, and one wishes to estimate $\theta_{0}$.
...
\times [0,1]$ and that $\Gbar_{0}(W) \equiv P_{0}(A=1|W)$ is bounded away from
...
\begin{equation*}\Gbar(W)  \equiv _{P}(A=1|W)\end{equation*}  is bounded  away
...
## Fluctuations, smoothness and `another_experiment` {#smooth-first-pass}
...
(ref:psi-approx-psi-one)  Evolution   of  statistical  mapping   $\Psi$  along
...
$P$.^[Interestingly,  if  a  fluctuation  $\{P_{h}  :  h  \in  H\}$  satisfies
\@ref(eq:score) for a direction $s$ such  that $s\neq 0$, $\Exp_{P}(s(O)) = 0$
and  $\Var_{P}  (s(O))  <  \infty$,  then $h  \mapsto  \Psi(P_{h})$  is  still
differentiable  at  $h=0$  with  a derivative  equal  to  \@ref(eq:derivative)
beyond fluctuations of the form \@ref(eq:fluct).]
...
for reasons that will be  clarified in Section \@ref(influence-curves).  In
the  more general  setting,  the  canonical gradient  may  be  defined as  the
minimizer of $D\mapsto \Var_{P} (D(O))$ over the set of all gradients.

It  is  not difficult  to  check  that  the  efficient influence  curve  of
statistical  mapping $\Psi$  \@ref(eq:psimap) at  $P \in  \calM$ can  be written  as
\begin{align}
D^{*}(P) & \equiv D_{1}^{*} (P) + D_{2}^{*} (P), \quad \text{where}  (\#eq:eif)\\
D_{1}^{*}(P) (O) &\equiv \Qbar(1,W) - \Qbar(0,W) - \Psi(P), \notag\\ 
D_{2}^{*}(P) (O) &\equiv \frac{2A-1}{\ell\Gbar(A,W)}(Y - \Qbar(A,W)).\notag 
\end{align}
...
## &#9761; \stixdanger{} Influence curves and the efficient influence curve {#influence-curves}
...
linear* with  *influence curve*  $\IC$.  We note  that weak  convergence of
asymptotically linear estimators is implied by the central limit theorem. That
is, if $\psi_n$  is asymptotically linear with influence  curve $\IC$, then
...
in law to a centered Gaussian distribution with variance $\Var_P(\IC(O))$.

As it happens,  influence curves of regular^[We can view  $\psi_{n}$ as the
by  product of  an  algorithm $\Psihat$  trained  on independent  observations
$O_{1}, \ldots, O_{n}$  drawn from $P$.  We say that  the estimator is regular
at $P$  if, for any direction  $s\neq 0$ such  that $\Exp_{P} (s(O)) =  0$ and
$\Var_{P} (s(O))  < \infty$ and fluctuation  $\{P_{h} : h \in  H\}$ satisfying
\@ref(eq:score), the estimator $\psi_{n,1/\sqrt{n}}$ of $\Psi(P_{1/\sqrt{n}})$
obtained by  training $\Psihat$  on independent observations  $O_{1}$, \ldots,
$O_{n}$    drawn    from    $P_{1/\sqrt{n}}$   is    such    that    $\sqrt{n}
(\psi_{n,1/\sqrt{n}} - \Psi(P_{1/\sqrt{n}}))$ converges in law to a limit that
does not depend on $s$.]  estimators  are intimately related to gradients.  In
fact, if $\psi_n$  is a regular, asymptotically linear  estimator of $\Psi(P)$
with influence curve $\IC$, then it must be true that $\Psi$ is a smooth
at $P$ and  that $\IC$  is a  gradient of $\Psi$  at $P$.  
...
*Cramér-Rao bound*.  Any  regular estimator that achieves  this variance bound
is  said  to be  *asymptotically  efficient*  at  $P$.  Since,  the  canonical
gradient is the  influence curve of an  asymptotically efficient estimator,
it  is  often  referred  to  as the  *efficient  influence  curve*.
...
influence curve. Moreover, we said that this influence curve must in fact
...
conditions, it is  possible to construct an estimator  with influence curve
...
## &#9761; \stixdanger{} Double-robustness {#double-robustness}
...
and *either* $\Qbar'  = \Qbar$ *or* $\Gbar' = \Gbar$,  then $\Rem_{P} (\Qbar',
\Gbar') =  0$ hence  $\Psi(P') = \Psi(P)$.   In words, if  $P'$ solves  the so
called $P$-specific  efficient influence curve  equation \eqref{eq:solves:eic}
and if, in addition, $P'$  has the same $\Qbar$-component or $\Gbar$-component
as $P$,  then $\Psi(P')  = \Psi(P)$  no matter  how $P'$  may differ  from $P$
otherwise.   This  property  is  useful  to  build  consistent  estimators  of
$\Psi(P)$.

## &#9881; \gear Double-robustness {#exo-double-robustness}

Confirm the double-robustness of the efficient influence curve by doing the following:
...
1. \tcg{Will modify  to reflect the final structure of  `LAW` methods, etc...}
   ...
   influence curve is zero. Repeat for several choices of `Gbar`.
...
parameter and  its efficient influence  curve. Because estimation  of these
...
$P_{n}$,  the empirical  measure, *i.e.*,  the law  consisting in  drawing one
among $O_{1}$, \ldots, $O_{n}$ with equal probabilities $n^{-1}$. Thus, we can
write $\Qbar_n = \hat{Q}(P_n)$.

## &#9881; \gear Algorithms {#exo-algorithms}
...
## &#9881; \gear Plug-in estimate {#exo-plug-in-estimate}
...
### &#9761; \stixdanger{} Analysis of plug-in estimator. \label{subsec:analysis-of-plug-in}
...
$\hat{P}_n$ a law in $\calM$ such that $\Qbar_{\hat{P}_n} = \Qbar_n$, 
$\Gbar_{\hat{P}_n} = \Gbar_n$ and $Q_{\hat{P}_n,W} = Q_{n,W}$. We will hence say that 
that such a law is *compatible* with $\Qbar_n$, $\Gbar_n$ and $Q_{n,W}$. 

We can apply equation (\ref{taylor_expansion}) substituting $\hat{P}_n$ for $P$: \begin{equation} \label{hard_to_study}
...
\end{equation*}
To facilitate our study of (\ref{hard_to_study}), we write \begin{align*}
...
The first term can be written $(P_n - P_0) D^*(P_0) \equiv n^{-1} \sum_{i=1}^n D^*(P_0)(O_i)$,
since $\Exp_{P_0}(D^*(P_0)) = 0$. Because this term is a sum of independent transformations of the data that have mean zero and finite variance, we can use standard tools such as the weak law of large numbers and central limit theorem to study its behavior. 

The term $R_{P_0}(\hat{P}_n)$  is an expectation involving the  product of two
...
$D^*(\hat{P}_n)$ and $D^*(P_0)$. Thus, we should hope that this term too will be negligible in
large samples. Formal conditions for the asymptotic negligibility of this term are provided
by results from empirical process theory\footnote{If $D^*(\hat{P}_n)$ falls in a $P_0$-Donsker 
class with probability tending to 1 and $P_0 \left( D^*(\hat{P}_n) - D^*(P_0) \right)^2$ converges to zero in probability as $n \rightarrow \infty$, then $(P_n - P_0) (D^*(\hat{P}_n) - D^*(P_0)) = o_{P_0}(n^{-1/2})$.}. We proceed assuming that such conditions are satisfied. 

Finally, we turn to the term $P_n D^*(\hat{P}_n)$. We will soon see that this term is the 
source of the poor behavior of the plug-in estimator. Notice that the data appear in this term 
twice, once through $\hat{P}_n$ and once through $P_n$ itself. Thus, the term involves a 
empirical average of a data-dependent transformation of the data. Such terms are difficult to 
analyze, and we have no reason to suspect that if $\hat{P}_n$ involves flexible regression 
techniques, this term will be asymptotically negligible. Indeed, we can study numerically 
study the behavior of this term via Monte Carlo simulation. The next code chunk analyzes its 
behavior. 
...
This simulation demonstrates that, as expected, $n^{1/2}P_n D^*(\hat{P}_n) \rightarrow \infty$ and thus is the source of the poor behavior of the plug-in estimate. 
...
is asymptotically linear if we can write $\psi_n = \psi_0 + \frac{1}{n} \sum_{i=1}^n \mbox{IF}(O_i) + o_{P_0}(n^{-1/2})$. Now, in the previous subsection we saw that, under
certain regularity conditions, the plug-in estimator could be written as \begin{equation} \label{pre_one_step}
  \Psi(\hat{P}_n) = \Psi(P_0) + \frac{1}{n} \sum_{i=1}^n D^*(P_0)(O_i) - \frac{1}{n} \sum_{i=1}^n D^*(\hat{P}_n)(O_i) + o_{P_0}(n^{-1/2}). 
\end{equation}
...
prevents the plug-in from attaining asymptotic linearity. Nevertheless, (\ref{pre_one_step}) implies
that \[
  \Psi(\hat{P}_n) + \frac{1}{n} \sum_{i=1}^n D^*(\hat{P}_n)(O_i) = \Psi(P_0) + \frac{1}{n} \sum_{i=1}^n D^*(P_0)(O_i) + o_{P_0}(n^{-1/2}). 
\]
...
\]
is an asymptotically linear estimator with influence curve equal to D^*(P_0). That is 
...
In particular, consider a $\hat{P}_n^*$ that is compatible with the estimates $\Qbar_n^*$, $\Gbar_n$, and $Q_{n,W}$, but moreover is such that $P_n D^*(\hat{P}_n^*) = 0$ (or at the very least equals to $o_{P_0}(n^{-1/2})$). If indeed, such estimates can be generated, then the 
plug-in estimator \[
...
estimate $\Qbar_n$ of $\Qbar_0$ and a distribution $\hat{P}_n^*$ that is compatible with 
$\Qbar_n$ and $Q_{n,W}$, 
\begin{align*}
  P_n D_1^*(\hat{P}_n^*) &= \frac{1}{n} \sum_{i=1}^n D_1^(\hat{P}_n^*)(O_i)\\
...
$Q_{0,W}$, by its very construction we will have that $P_n D_1^*(\hat{P}_n^*) = 0$. 
...
It thus remains to devise a strategy for ensuring that $P_n D_2^*(\hat{P}_n^*) = 0$. To 
...
such that for any law $\hat{P}_n^*$ that is compatible with this revised estimate,
$P_n D^*(\hat{P}_n^*) = 0$. Because $\Qbar_n$ is our best (initial) estimate of true 
...
estimate of $\Qbar_0$ such that for any $\hat{P}_n^*$ that is compatible with such an estimate $P_n D^*(\hat{P}_n^*) = 0$. Have we achieved this goal as well? Consider the fact that because the minimum value of $\bbD_n(\Qbar_{n,\delta})$ is achieved at $\delta = 0$, it must be true that the derivative of $\bbD_n(\Qbar_{n,\delta})$ with respect to $\delta$ is zero when evaluated at $\delta = 0$. That is, \begin{equation} \label{deriv_equals_zero}
...
$P_n D_{2}^{*}(\hat{P}^*_n)$ for a $\hat{P}_n^*$ that is compatible with $\Qbar_{n,\epsilon_n}$ and $\Gbar_n$. But then equation (\ref{deriv_equals_zero}) also implies that $P_n D_{2}^{*}(\hat{P}^*_n) = 0$! 
...
Assuming that the regularity conditions discussed in Section \ref{subsec:analysis-of-plug-in}
are satisfied for $\Qbar_{n,\epsilon_n}$, then by (\ref{pre_one_step}), \[
  \psi_n^* = \Psi(\hat{P}_n^*) = \int (\Qbar_{n,\epsilon_n}(1,w) - \Qbar_{n,\epsilon_n}(0,w)) dQ_{n,W} , 
\]
...
and hypothesis tests exactly as in Section \ref{subsec:analysis-of-plug-in}, but replacing
...
1. \tcg{Confirm the derivative of log-likelihood loss along submodel?}

2. \tcg{Consider a linear submodel through $\Qbar_n$, $\calQ_2{\Gbar_n} = \{\Qbar_{n,\epsilon}(A,W) = \Qbar_{n}(A,W) + \epsilon (2A-1)/\ell\Gbar_n(A,W) : \epsilon \in ]-10,10[ \}$. Plot regression functions corresponding to $\epsilon = -5, 0, 5$ and $a = 0, 1$. Do you notice anything wrong with these regressions? (Here I'm trying to show that this is not a valid submodel. But maybe for small enough parameter space on $\delta$ we could end up with
a valid submodel? But maybe then the MLE would be at the boundary and we wouldn't have that
the equation is satisfied necessarily. So maybe this isn't a good route to go down with a problem?)}

3. \tcg{Depending on whether we decide that this is a reasonable train of thought, I would
have them show that for squared error loss that we generate the proper score, so that 
at least we will satisfy the equation. Could then have them do a simulation and see how often
we end up with an estimate of $\psi_0$ that is outside the parameter space.}



## Inference assuming the mechanism of action known, or not, first pass {#known-gbar-first-pass}

\tcg{I think it makes sense to remove this, unless we want to add a section to 
the simple estimation strategies section above that talks about IPTW estimation.}
...
interested in the *law* of $\psi_{n}^{b}$,  the next chunk of code constitutes
`iter =` `r  iter` independent samples of independent  observations drawn from
$P_{0}$, each  consisting of $n$  equal to  `nrow(obs)/iter =` `r  nrow(obs) /
iter`  data points,  and computes  the  realization of  $\psi_{n}^{b}$ on  all
samples.
...
procedures  that involve  penalization (*e.g.*  the LASSO)  or aggregation  of
competing   estimators  (*via*   stacking/super  learning)   --  see   Section
...
because  the *inverse*  of  each $\ell\Gbar_{0}(A_{i},W_{i})$  appears in  the
...
## &#9881; \gear A nice title... {#exo-a-nice-title}
...
6.  &#9761; \stixdanger{} Drawing inspiration from \eqref{eq:v:n:b}, one may consider
