...
Let us compute $\psi_{n}^{d}$ $\psi_{n}^{e}$ and on the same `iter = ` 
...
Finally,   `bias_de`   reports   amounts   of  bias   (at   the   renormalized
scale**s**). There is one value of bias  for each combination of *(i)* type of
the estimator (`d` or `e`) and  *(ii)* how the renormalization is carried out,
