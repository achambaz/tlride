...
\begin{equation}
\calQ(\Qbar,\Gbar)     \triangleq     \left\{(a,w)     \mapsto
\Qbar_{h}(a,w)   \triangleq   \expit\left(\logit\left(\Qbar(a,w)\right)  +   h
\frac{2A  -  1}{\ell   \Gbar(a,w)}  \right)  :  t   \in  \bbR\right\}  \subset
\calQ. (\#eq:Q-fluct)
\end{equation}
...
data-driven  $h_{n} \in  \bbR$. We  will  clarify why  we  do so  [at a  later
time](#fluct-justification) 
...
candidates <- seq(-1e-2, 1e-2, length.out = 1e2)
W <- obs[1:1e3, "W"]
A <- obs[1:1e3, "A"]
Y <- obs[1:1e3, "Y"]
...
minimizer  $h_{n}$  equals   0,  that  is  (see   [here](#fluct-score)  for  a
justification of the last but one equality below),

\begin{align*}      \frac{d}{d       h}      &       \left.       \Exp_{P_{n}}
...
-\frac{1}{n}\sum_{i=1}^{n}  \left(\frac{Y_{i}}{\Qbar_{n}^{*}(A_{i}, W_{i})}  -
\frac{1    -   Y_{i}}{1    -   \Qbar_{n}^{*}(A_{i},    W_{i})}\right)   \times
\left.\frac{d}{d   h}   \Qbar_{n,h}^{*}(A_{i},   W_{i})\right|_{h=0}   \\   &=
-\frac{1}{n}\sum_{i=1}^{n}       \frac{Y_{i}      -       \Qbar_{n}^{*}(A_{i},
W_{i})}{\Qbar_{n}^{*}(A_{i},  W_{i})  \times  \left(1  -  \Qbar_{n}^{*}(A_{i},
W_{i})\right)} \left.\frac{d}{d  h} \Qbar_{n,h}^{*}(A_{i}, W_{i})\right|_{h=0}
\\  &=   -\frac{1}{n}\sum_{i=1}^{n}  \frac{2A_{i}   -  1}{\ell\Gbar_{n}(A_{i},
W_{i})} \left(Y_{i} - \Qbar_{n}^{*}(A_{i}, W_{i})\right) = 0.\end{align*}

 

2. that the derivative of (the real-valued function over $\bbR$) $h \mapsto
\Exp_{P_n} \left(L_{y} (\Qbar_{n,h})(O)\right)$ equals 0 when evaluated at its
minimizer $h_{n}$, and to compute the said gradient.
...
psi_trees_epsilon <- sapply(epsilon, function(h) {
  apply_targeting_step(head(obs, 1e3), Gbar_hat_w,  Qbar_trees, epsilon = h) %>%
    select(psi_n, crit_n) %>% as.matrix
})
idx_trees <- which.min(abs(psi_trees_epsilon[2, ]))

psi_kknn_epsilon <- sapply(epsilon, function(h) {
  apply_targeting_step(head(obs, 1e3), Gbar_hat_w, Qbar_kknn, epsilon = h) %>%
    select(psi_n, crit_n) %>% as.matrix
})
