...
For instance, `estimate_QW` is an algorithm  $\Algo$ for the estimation of the
marginal  law  of  $W$  under  $P_{0}$  (to  see  its  man  page,  simply  run
...
We  acted  as  oracles when  we  specified  the  working  model: it  is  *well
specified*, *i.e.*, it happens that $\Gbar_{0}$ is the unique minimizer of the
risk  entailed  by  $L_{a}$  over  $\calF_{1}$:  \begin{equation*}\Gbar_{0}  =
...
model-based (see Section \@ref(nuisance-Gbar)) or machine learning-based. 
...
## `Qbar`, machine learning-based algorithms {#nuisance-Qbar-ml}
