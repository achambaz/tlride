...
namely, the we merely call the function `apply_one_step_correction`
...
method for  selecting an estimator of  $\Qbar_0$ that is "near  to" $\Qbar_n$,
but is  such that  for any law  $P_n^*$ that is  compatible with  this revised
estimator, $P_n D^*(P_n^*) =  0$.^[or $P_n D^*(P_n^*) = o_{P_0}(1/\sqrt{n})$.]
Because  $\Qbar_n$ is  our  best  (initial) estimator  of  $\Qbar_0$, the  new
estimator that  we shall  build should be  *at least as  good* an  estimate of
