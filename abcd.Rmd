...
- Section   \@ref(simple-strategies)   present   two   simple   inference
  strategies. Easier,  the first one  can be  carried out in  situations where
  $\Gbar_{0}$ is  already known to  the statistician. A little  more involved,
  the second one does not need such prior knowledge.

- Section...

# Two simple inference strategies {#simple-strategies}

## A cautionary parenthesis
...
2. &#9761;  \stixdanger{}   Argue   how  the   delta-method  yields   that
   $\sqrt{n}(\psi_{n}^{a}  - \Phi(P_{0}))$  converges in  law to  the centered
   Gaussian law with a variance that can be estimated with 
\begin{equation}
v_{n}^{a} \equiv \nabla f(X_{n}) \times V_{n} \times \nabla f(X_{n})^{\top}. (\#eq:v-n-a)
\end{equation}
...
## Assuming the mechanism of action known {#known-gbar-first-pass}

### A simple IPTW substitution estimator
...
build the to  estimators on each of  them (fourth and fifth  lines). After the
first series of commands the object `psi_hat_ab`, a `tibble`, contains 
`r 2*iter` rows and five columns.  For each smaller data set (identified by its
`id`), two rows contain the values  of either $\psi_{n}^{a}$ (if `type` equals
`a`) or $\psi_{n}^{b}$ (if `type` equals `b`).
...
`bias_ab` can be interpreted as an amount of bias (at the renormalized scale).
...
psi_hat_de <- learned_features_fixed_sample_size %>%
  ...
  mutate(std_d = sd(est_d),
         std_e = sd(est_e),
         clt_d = (est_d - psi_zero) / std_d,
         clt_e = (est_e - psi_zero) / std_e) %>% 
  gather("key", "value", -id) %>%
...
root_n_bias <- learned_features_varying_sample_size %>%
  unnest(blip_QW_d, blip_QW_e) %>%
  group_by(block) %>%
  summarize(clt_d = sqrt(n()) * (mean(blip_QW_d) - psi_zero),
            clt_e = sqrt(n()) * (mean(blip_QW_e) - psi_zero)) %>%
