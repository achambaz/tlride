...
use in the next subsections.

### `QW`

For instance, `estimate_QW` is an algorithm for the estimation of the marginal
law of $W$ under $P_{0}$ (to see its man page, simply run `?estimate_QW`). The
following chunk  of code  estimates $Q_{0,W}$  based on the  $n =  1000$ first
observations in `obs`:
...
It is easy to sample independent observations from `QW_hat`. To do so, we make
a copy of `experiment` and replace its marginal law of $W$ with that described
by `QW_hat`:

(ref:estimate-QW-two)   Histogram   representing   1000   observations   drawn
independently from `QW_hat`.

```{r estimate-QW-two, fig.cap = '(ref:estimate-QW-two)'}
empirical_experiment <- copy(experiment)
alter(empirical_experiment, QW = QW_hat)
sample_from(empirical_experiment, 1e3) %>% as.tibble %>%
  ggplot(aes(x = W)) + stat_bin(bins = 40)
```
...
learned_features_fixed_sample_size <-
  ...
  mutate(Qbar_hat_d = map(obs, ~ estimate_Qbar(., algorithm = working_model_Q_one)),
         Qbar_hat_e = map(obs, ~ estimate_Qbar(., algorithm = bstTree_algo, ## bstTree
                                               trControl = bstTree_control, ## bstTree
                                               tuneGrid = bstTree_grid))) %>% ## bstTree
