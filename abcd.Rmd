...
(ref:grid-search) Legend.

```{r grid-search, fig.cap = '(ref:grid-search)'}
candidates <- seq(-1e-1, 1e-1, length.out = 1e2)
W <- obs[1:1e3, "W"]
A <- obs[1:1e3, "A"]
Y <- obs[1:1e3, "Y"]
...
risk %>% enframe %>%
  ...
  geom_vline(xintercept = candidates[idx_min]) +
  geom_hline(yintercept = risk[idx_min]) +
  labs(y = "empirical logistic risk")
