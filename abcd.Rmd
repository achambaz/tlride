...
properties   inherited   from   this   insightful   representation   (Sections
\@ref(smooth) and \@ref(double-robustness)). We then  turn to the inference of
the  parameter of  interest.  We  first introduce  and comment  upon a  simple
inference strategy assuming  provisionally that a relevant feature  of the law
of  the data  is  known  to us  (Section  \@ref(simple-strategy)). Second,  we
present the notion  of nuisance parameters and adopt an  algorithmic stance on
their estimation  (Section \@ref(nuisance)).  Third, we introduce  and comment
upon two  "naive" inference strategies (Section  \@ref(naive-estimators)), the
one-step  correction procedure  (Section  \@ref(one-step))  and, finally,  the
targeted minimum  loss estimation procedure  tailored to the inference  of the
parameter of main interest.  In the appendix, we collect our notation (Section
...
\times [0,1]$ and that $\Gbar_{0}(W) \triangleq \Pr_{P_{0}}(A=1|W)$ is bounded away from
...
stage, the  estimators are substituted  for their theoretical  counterparts in
the definition of $\Psi(P_{0})$, thus yielding estimators of $\Psi(P_{0})$.
...
fluctuation model for $\Qbar^a \triangleq \Qbar(a,\cdot)$:
\begin{equation} 
\calQ^a(\Qbar^a)
\triangleq      \left\{w       \mapsto      \Qbar_{h}^a(w)      \triangleq
\expit\left(\logit\left(\Qbar(a,w)\right) +  h
\right) : h \in \bbR\right\} \subset \calQ. (\#eq:Q-fluct-alt) 
\end{equation}
Let 
\begin{equation*}
  \calQ^{\text{alt}}(\Qbar)   \triangleq   \left\{(a,w)  \mapsto   \Qbar_{h_0,
  h_1}(a,w) \triangleq a \times
...
\begin{equation*}
   L_{y,\Gbar}^{a} (f) (A,W) \triangleq \frac{\one\{A = a\}}{\ell\Gbar(A, W)} L_{y}
   (f)(O)
\end{equation*}
