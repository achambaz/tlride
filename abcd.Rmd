...
## The `rgttmle` package {#gttmle-package}

The  computational illustrations  will  be developed  based  on the  companion
package `rgttmle`. Make  sure you have installed it,  for instance by
running the following chunk of code:

```{r install-package, eval = FALSE}
devtools::install_github("achambaz/gttmle/rgttmle")
```
...
Let us run the example built into the `rgttmle` package:
...
# &#9881; Make your own experiment {#exo-make-own-experiment}
...
# (PART\*) The parameter of interest {-}
...
# &#9881; An alternative parameter of interest {#exo-alternative-parameter-first-pass}
...
1.  &#x2621; Prove \@ref(eq:alter-gamma-zero).
...
Interestingly, the  empirical measure $P_{n}$ ^[The  empirical measure $P_{n}$
is  the law  such  that *(i)*  $X$  drawn  from $P_{n}$  takes  its values  in
$\{X_{1}, \ldots, X_{n}\}$, and *(ii)* $X=X_{i}$ with probability $n^{-1}$] is
an  element of  $\calM$. Therefore,  the statistical  mapping $\Theta$  can be
...
## The value of the statistical mapping at another experiment

When  we  ran   `example(rgttmle)`  earlier,  we  created   an  object  called
`another_experiment`:
...
# &#9881; Alternative statistical mapping {#exo-alternative-parameter-second-pass}
...
# &#9881; Alternative representation {#exo-alternative-parameter-third-pass}
...
*smooth*   statistical   mapping. 

## Fluctuations {#fluctuations}


To discuss how $\Psi(P)$ changes for distributions that *get nearer* to $P$ in
the model, we require a more concrete notion of *getting-nearness*.


To that end, consider the law 
...
# &#x2621; Second pass {#smooth-second-pass}
...
\begin{equation}                   \Exp_{P}                  \left(D^{*}(P)(O)
s(O)\right).(\#eq:derivative)\end{equation}
...
surprising given the previous parallel  drawn to Euclidean geometry.  However,
it is important  to remember that the model dictates  fluctuations of $P$ that
are valid submodels with respect to  the full model.  In turn, this determines
the possible  directions from which we  may approach $P$.  Thus,  depending on
the  direction,  \@ref(eq:derivative)  may  hold  with  different  choices  of
$D^*$.].  However, in the special case that the model is nonparametric, only a
single  gradient exists,  which  is  sometimes referred  to  as the  canonical
gradient. In the  more general setting, the canonical gradient  may be defined
as the minimizer of $D\mapsto \Var_{P} (D(O))$ over the set of all gradients.
...
(eic_another_experiment(two_obs_another_experiment))
```
...
## \textdbend Analysis of plug-in estimator. \label{subsec:analysis-of-plug-in}
...
The term $R_{P_0}(\hat{P}_n)$ is an expectation involving
the product of two differences, one between $\Qbar_n$ and $\Qbar_0$, the other between 
$\Gbar_n$ and $\Gbar_0$. Intuitively, we expect that if $\Qbar_n$ and $\Gbar_n$ are good 
estimates of $\Qbar_0$ and $\Gbar_0$, respectively, then the expectation of a product of differences between these quantities should be small, and, we should hope, negligible in large samples. Formal conditions for the asymptotic negligibility of this term can be made via the Cauchy-Schwarz inequality\footnote{For any measurable function $f:\calO \to \bbR$, we denote the $L_2(P)$-norm of $f$ by $|f|_{P} \equiv \Exp_{P} \left( f(O)^{2} \right)^{1/2}$. Assume that for $a= 0,1$, $\ell\Gbar_{n}(a,W) > \delta$ almost everywhere $Q_{0,W}$. Then the Cauchy-Schwarz inequality implies that for $a = 0,1$, $\Rem_{P_0}(\hat{P}_n) \le \frac{2}{\delta} \text{max}_{a=0,1} \left( | \Qbar_n(a,\cdot) - \Qbar_0(a,\cdot)|_{P_0} \right) |\Gbar_n - \Gbar_0|_{P_0}$. Therefore, if for $a=0,1$, $|\Qbar_n(a,\cdot) - \Qbar_0(a,\cdot)|_{P_0} = o_{P_0}(n^{-1/4})$ and $|\Gbar_n - \Gbar_0|_{P_0} = o_{P_0}(n^{-1/4})$ then $\Rem_{P_0}(\hat{P}_n) = o_{P_0}(n^{-1/2})$.}.
...
hope that the new estimate is at least *no further from $\Qbar_0$* than $\Qbar_n$. Let's 
use $\Qbar_{n,\epsilon_n}$ to denote the new estimate of $\Qbar_0$. 
...
&\hspace{0.1in} = \frac{1}{n} \sum_{i = 1}^n \frac{2A_i - 1}{\ell \Gbar_n(A_i, W_i)} \left(Y_i - \Qbar_{n,\delta = 0}) \\ 
&\hspace{0.1in} = \frac{1}{n} \sum_{i = 1}^n \frac{2A_i - 1}{\ell \Gbar_n(A_i, W_i)} \left(Y_i - \Qbar_{n,\epsilon_n}) 
\end{align*}
...
solves the efficient influence function equation. 
