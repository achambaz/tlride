...
\notag &= -\frac{1}{n} \sum_{i=1}^n D^*(\Phat_n)(O_i) + \frac{1}{n} \sum_{i=1}^n D^*(P_0)(O_i)  + o_{P_0}(1/\sqrt{n}). 
\end{align}
...
In a hurry, let us demonstrate how  the one-step correction unfolds when it is
applied to the estimators $\psi_{n}^{d}$ and $\psi_{n}^{e}$ that we studied in
Section \@ref(Gcomp-estimator). This is what the next chunk of code does.

(ref:one-step-one)   Kernel   density   estimators   of   the   law   of   two
one-step-G-computation estimators  of $\psi_{0}$  (recentered with  respect to
$\psi_{0}$,  and   renormalized).   The   estimators  respectively   hinge  on
algorithms  $\Algo_{\Qbar,1}$   (d)  and  $\Algo_{\Qbar,\text{kNN}}$   (e)  to
estimate $\Qbar_{0}$, and on one-step correction.  Two renormalization schemes
are considered, either based on an estimator of the asymptotic variance (left)
or  on  the   empirical  variance  computed  across   the  `iter`  independent
replications of the estimators (right).  We emphasize that the $x$-axis ranges
differ between the left and right plots.


```{r one-step-one, fig.cap = '(ref:one-step-one)'}
apply_one_step <- function(obs, Gbar_hat, Qbar_hat, psi_hat) {
  eic <- function(dat) {
    if (length(intersect(c("W", "A", "Y"), names(dat))) == 3) {
      stop(stringr::str_c("Argument 'dat' of ",
                          deparse(substitute(this)),
                          " should contain columns named 'W', 'A' and 'Y'.\n"))
    } 
    QAW <- Qbar_hat(dat[, c("A", "W")])
    QoneW <- Qbar_hat(cbind(A = 1, W = dat[, "W"]))
    QzeroW <- Qbar_hat(cbind(A = 0, W = dat[, "W"]))
    GW <- Gbar_hat(dat[, "W", drop = FALSE])
    lGAW <- dat[, "A"] * GW + (1 - dat[, "A"]) * (1 - GW)
    out <- (QoneW - QzeroW - psi_hat) + (2 * dat[, "A"] - 1) / lGAW * (dat[, "Y"] - QAW)
    out <- as.vector(out)
    return(out)
  }
  eic_obs <- eic(obs)
  psi_n <- psi_hat + mean(eic_obs)
  sig_n <- sd(eic_obs)/sqrt(nrow(obs))
...
psi_hat_de %>%
  ...
  group_by(type) %>%
  summarize(sd = mean(sig_n) * sqrt(n()), ## * ifelse(str_detect(type, "one_step"), 1, NA)),
            se = sd(psi_n),
            mse = mean((psi_n - psi_zero)^2) * n())
```

The  `sd`  (*estimator*  of  the   asymptotic  standard  deviation)  and  `se`
(*empirical* standard  deviation) entries  for type `d_one_step`  are similar:
this  indicates  that  the  inference   of  the  asymptotic  variance  of  the
`d`-variant of the  one-step estimator based on the influence  curve is rather
accurate.   On  the  contrary,  the influence  curve-based  estimator  of  the
asymptotic variance  is clearly  anti-conservative for type  `e_one-step`.  As
for the mean  square error, it is  diminished by the one-step  update for both
types `d`  and `e`,  the `e_one_step` estimator  exhibiting the  smallest mean
square error.
