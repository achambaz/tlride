...
This subsection is very similar to its counterpart for the IMPT estimator, see
...
Because $\Qbar_{n,1}$ minimizes the  empirical risk over a finite-dimensional,
identifiable, and **well-specified** working  model, $\sqrt{n} (\psi_{n}^{d} -
\psi_{0})$ converges in law to a centered Gaussian law.\index{identifiability}
Moreover, the asymptotic  variance of $\sqrt{n} (\psi_{n}^{d}  - \psi_{0})$ is
**anti-conservatively**^[In words,  $v_{n}^{d}$ converges to a  lower-bound of
the      true     asymptotic      variance.]\index{conservative}     estimated
with\index{well/mis-specified}

\begin{align*}            v_{n}^{d}            &\equiv            \Var_{P_{n}}
\left(\Qbar_{n,1}(1,\cdot) - \Qbar_{n,1}(0,\cdot)\right) \\ &= \frac{1}{n}
\sum_{i=1}^{n}\left(\Qbar_{n,1}(1,W_{i})         -        \Qbar_{n,1}(0,W_{i})
-\psi_{n}^{d}\right)^{2}.  \end{align*} 
...
well-specified, fact that cannot be true in the present case).
...
Let us  compute $\psi_{n}^{d}$  $\psi_{n}^{e}$ and  on the same  `iter =  ` `r
iter` independent samples of independent observations drawn from $P_{0}$ as in
...
$\sqrt{v_{n}^{d}}/\sqrt{n}$,  the   `tibble`  also  includes  the   values  of
$\sqrt{n}/\sqrt{v^{*}} (\psi_{n}^{d} - \psi_{0})$  where the estimator $v^{*}$
of  the   asymptotic  variance  is   computed  *across  the   replications  of
$\psi_{n}^{d}$*.

ANTOINE

Finally, `bias_de` can be interpreted as an amount of bias (at the
renormalized scale).
...
```{r fig.cap = '(ref:unknown-Gbar-seven)', eval = FALSE}
clt_c <- psi_hat_abc %>% filter(type == "c") %>%
  mutate(clt = clt * sig_n /  emp_sig_n)
